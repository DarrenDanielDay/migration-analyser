...
class ProjectStatusCommand implements Command {
  ...
  private getVersionItem(): QuickPickItem {
    return {
      label: localize(
        "projectQuickPick.version.label",
        "Select TypeScript Version..."
      ),
      description: localize(
        "projectQuickPick.version.description",
        "[current = {0}]",
        this._client.apiVersion.displayName
      ),
      run: () => {
  ...
  private getProjectItem(
    ...
    return {
      label: localize(
        "projectQuickPick.version.goProjectConfig",
        "Open tsconfig"
      ),
      description:
        info.type === ProjectInfoState.Type.Resolved
...
export default class VersionStatus extends Disposable {
  ...
  private async updateStatus() {
    ...
    if (isTypeScriptDocument(doc)) {
      const file = this._client.toOpenedFilePath(doc, {
