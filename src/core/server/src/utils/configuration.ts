...
export class ImplicitProjectConfiguration {
  ...
  constructor(configuration: vscode.WorkspaceConfiguration) {
    ...
    this.experimentalDecorators = ImplicitProjectConfiguration.readExperimentalDecorators(
      configuration
    );
    this.strictNullChecks = ImplicitProjectConfiguration.readImplicitStrictNullChecks(
      configuration
    );
    this.strictFunctionTypes = ImplicitProjectConfiguration.readImplicitStrictFunctionTypes(
      configuration
    );
  }
...
export class TypeScriptServiceConfiguration {
  ...
  public readonly includePackageJsonAutoImports:
    | "auto"
    | "on"
    | "off"
    | undefined;
  ...
  private constructor() {
    ...
    this.implictProjectConfiguration = new ImplicitProjectConfiguration(
      configuration
    );
    this.disableAutomaticTypeAcquisition = TypeScriptServiceConfiguration.readDisableAutomaticTypeAcquisition(
      configuration
    );
    this.separateSyntaxServer = TypeScriptServiceConfiguration.readUseSeparateSyntaxServer(
      configuration
    );
    ...
    this.watchOptions = TypeScriptServiceConfiguration.readWatchOptions(
      configuration
    );
    this.includePackageJsonAutoImports = TypeScriptServiceConfiguration.readIncludePackageJsonAutoImports(
      configuration
    );
    this.enableTsServerTracing = TypeScriptServiceConfiguration.readEnableTsServerTracing(
      configuration
    );
  }
  ...
  private static readWatchOptions(
    ...
    return configuration.get<protocol.WatchOptions>(
  ...
  private static readEnableTsServerTracing(
    configuration: vscode.WorkspaceConfiguration
  ): boolean {
    return configuration.get<boolean>(
