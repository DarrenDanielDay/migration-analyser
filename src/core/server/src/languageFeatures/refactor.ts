...
class DidApplyRefactoringCommand implements Command {
  ...
  public async execute(args: DidApplyRefactoringCommand_Args): Promise<void> {
    ...
    if (renameLocation) {
      ...
      if (
        args.codeAction.document.uri.scheme !== fileSchemes.walkThroughSnippet
      ) {
...
class InlinedCodeAction extends vscode.CodeAction {
  ...
  public async resolve(token: vscode.CancellationToken): Promise<undefined> {
    ...
    // Resolve
    this.edit = InlinedCodeAction.getWorkspaceEditForRefactoring(
      this.client,
      response.body
    );
...
class TypeScriptRefactorProvider
  ...
  constructor(
    ...
    commandManager.register(
      new SelectRefactorCommand(this.client, didApplyRefactoringCommand)
    );
  }
  ...
  private convertApplicableRefactors(
    ...
    for (const info of body) {
      if (info.inlineable === false) {
        const codeAction = new SelectCodeAction(
          info,
          document,
          rangeOrSelection
        );
  ...
  private refactorActionToCodeAction(
    ...
    const codeAction = new InlinedCodeAction(
      this.client,
      action.description,
